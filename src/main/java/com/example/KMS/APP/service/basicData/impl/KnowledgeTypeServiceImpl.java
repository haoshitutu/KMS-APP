...
import com.example.KMS.APP.mapper.KnowledgeTypeMapper;
...
public class KnowledgeTypeServiceImpl implements KnowledgeTypeService {
    ...
    @Override
    public List<KnowledgeType> getAllKnowledgeTypes() {
        try {
            return knowledgeTypeMapper.selectAll();
        } catch (Exception e) {
            log.error(String.format("获取全部知识类型基础数据失败！--%s", e.getMessage()));
            throw new BusinessException(String.format("获取全部知识类型基础数据失败！--%s", e.getMessage()));
        }
    }
}